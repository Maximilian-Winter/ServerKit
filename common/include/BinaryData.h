...
namespace NetworkMessages
{
    ...
    private:
        ...
        static ByteVector read_byte_vector(const ByteVector& data, size_t& offset)
        {
            auto size = read_bytes<uint32_t>(data, offset);
            ByteVector vec;
            vec.reserve(size);
            vec.insert(vec.end(), data.begin() + offset, data.begin() + offset + size);
            offset += size;
            return vec;
        }
